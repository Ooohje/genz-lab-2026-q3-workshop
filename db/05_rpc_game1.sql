...
--   · is_lie 와 득표는 그 팀이 리빌 단계에 들어간 뒤에만 내보낸다.
--   · 완료 판정 분모는 스냅샷이 아니라 호출 시점의 편성으로 매번 계산한다.
--   · 턴 전환은 get_g1_view 안에서 판정한다. 발표자 폰이 죽어도 다른 팀원의
--     폴링이 대신 턴을 넘겨주므로 팀이 멈추지 않는다.
-- =====================================================================
...
begin
  ...
  -- 팀이 'done' 으로 끝난 뒤 지각자가 문장을 쓰고 합류하면 발표자가 새로 생긴다.
