...
3. **테스트·데모 데이터 정리** (아래 목록). 실명단 업로드 직전에. **개인정보 1건 포함 — 아래 참조.**
4. 리허설(D-3~D-1) — 폰 2대 이상으로 전 구간.

### 현재 DB 상태 — 전부 테스트·데모 데이터다. 실운영 전 정리 대상

2026-09-02 README 스크린샷 촬영을 위해 데모 한 벌을 심었다. 실제 참가자 데이터는 없다 — 아래 ⚠ 1건만 예외다.

| 테이블 | 현재 | 내용 |
|---|---|---|
| `teams` | 12행 (활성 10) | 1~10번 = 데모 조 이름(`반짝이는 팀` 등). **98·99번** = `테스트조B(삭제예정)` · `테스트조(삭제예정)`, 비활성 |
| `participants` | 68행 (활성 61) | `demo01`~`demo61` = 데모 61명. 나머지 7행은 98·99조 테스트 계정, 전부 비활성 |
| `statements` | 264행 / 66명 | 데모 × 4 + 테스트 잔여. `demo61` 만 미작성 — 빈 작성 화면 캡처용이므로 그대로 둔다 |
| `votes_3t1f` | 13행 | 게임 1 검증 잔여 |
...
| 명단 CSV (Knox ID, 이름, 조) | ⬜ |
| 테스트·데모 데이터 | 2026-09-02 검증 + README 촬영으로 `teams` 12 · `participants` 68 · `statements` 264 · `questions` 8 · `answers` 412 가 쌓여 있다. 상세는 맨 위 "현재 DB 상태" 참조. 실명단 업로드 전에 정리 |
| 문항 8개 최종본 | ⬜ |
| 조 이름 10개 | ⬜ |
