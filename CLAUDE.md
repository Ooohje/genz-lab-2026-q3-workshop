...
**8단계 구현 + 서버 로직 전 구간 검증 완료. 남은 것은 내용물과 실기기 리허설이다.**
...
| 검증 완료 | 로그인·정규화 · Realtime(실기기 2대) · **게임 1 턴 전환 4턴 전 구간** · **게임 2 출제~채점~리더보드** · 스크린 RPC 3종 · 관리자 22개 PIN 가드 전수 · RLS 잠금 |
| **미검증** | **브라우저 실화면**(스크린 1920×1080 · 참여자 폰 · 관리자) · 실기기 리허설 |
...
1. **브라우저 실화면 확인** — 서버 로직은 다 맞는데 화면을 아직 눈으로 못 봤다.
   `npm run dev` → 스크린은 `#/screen?phase=lobby` 처럼 phase 강제로 DB 안 건드리고 볼 수 있다.
2. 사용자에게 **문항 8개 · 명단 CSV · 조 이름 10개** 를 받아 입력. 이게 없으면 리허설을 못 한다.
3. **테스트 데이터 정리** (아래 목록). 실명단 업로드 직전에.
4. 리허설(D-3~D-1) — 폰 2대 이상으로 전 구간.

### 현재 DB 상태 — 전부 테스트 데이터다. 실운영 전 정리 대상

- `teams` **98·99번** (`테스트조B(삭제예정)` · `테스트조(삭제예정)`)
- `participants` 7행 — `test.claude` `ㅇㅇ` `ii` `js60.oh` `jj60.` 은 99조, `ㄷㄷ` 은 98조, `skeka` 는 비활성(이탈 테스트로 꺼둠)
- `statements` 5인분 · `votes_3t1f` · `answers` 검증 과정에서 쌓인 것
- `questions` **4개** (테스트 문항. 실제 8개로 교체할 것)
- `game_state.phase` = `game2_answer`, `current_question_id` = 3 (정리하려면 `lobby` 로)

정리는 `admin_reset_game` 또는 `db/00_reset.sql`. **둘 다 파괴적이므로 실행 전 확인받는다.**
...
**db/ 의 SQL 00~07 은 2026-09-02 에 전부 적용 완료했다.** RPC 38개가 실동작 검증까지 끝났다 — 문장 저장·셔플·입력검증, 게임 1 턴 전환 4턴 전 구간(자동 리빌 · 5초 후 전환 · 재선택 upsert · 발표자 단축 · 지각자 합류 · 발표자 이탈 건너뛰기 · 가드 6종), 게임 2 출제~채점~리더보드(스피드 보너스 차등 · 1회 확정 · `TOO_LATE` · 조 이동 시 분모 자동 추종), 관리자 22개 PIN 가드 전수.

**서버 로직은 검증됐지만 브라우저 실화면은 아직 안 봤다.** 다음 세션의 첫 일이다.
...
| 명단 CSV (Knox ID, 이름, 조) | ⬜ |
| 테스트 데이터 | 2026-09-02 검증으로 `teams` 98·99, `participants` 7행, `statements` 5인분, `questions` 4개, 투표·답안이 쌓여 있다. 상세는 맨 위 "현재 DB 상태" 참조. 실명단 업로드 전에 정리 |
| 문항 8개 최종본 | ⬜ |
| 조 이름 10개 | ⬜ |
