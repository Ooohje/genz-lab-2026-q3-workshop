...
### 6. 부가 투표 — 게임과 별개로 붙는 것들

워크샵 게임 진행(`game_state`)과 완전히 분리된, 필요할 때마다 추가하는 투표들이다. 로그인은 게임과 같은 Knox ID를 그대로 쓴다.

| 라우트 | 내용 | 재선택 | 결과 공개 |
|---|---|---|---|
| `#/dinner` | 석식 참석 여부(참석/미참석) | 가능 | 관리자만 |
| `#/poll/best-team` | 팀 발표 투표 — 1~10팀 발표 중 가장 좋았던 팀. **본인 팀은 선택 불가**(서버가 다시 검증한다) | 가능 | 순위만 스크린에, 득표수는 관리자만 |
| `#/poll/trenderZ` | "2026 하반기 트렌더즈 현장 접수" — 참여/미참여 | 가능 | 관리자만(참여자 명단 CSV 다운로드 가능) |

관리자 대시보드(`#/admin`)에 각 투표의 집계 패널과 초기화 버튼이 있다. **팀 발표 투표만 스크린 공개 버튼이 따로 있다** — 누르면 빔프로젝터(`#/screen`)에 현재 진행 중인 phase와 무관하게 순위 화면이 뜬다. 이때도 득표수는 절대 내려가지 않고 순서만 나간다(`get_poll_rank` RPC가 숫자를 아예 계산에서 뺀다).

새 투표를 추가할 때는 `db/09_polls.sql`의 `poll_votes` 테이블을 재사용한다 — `src/lib/polls.js`에 항목을 하나 늘리고, 같은 SQL 파일의 `submit_poll_vote` / `admin_poll_tally` / `get_poll_rank` 안 분기에 `poll_id`만 추가하면 된다.

---

## 하나의 앱, 네 개의 화면

반응형이 아니라 기준 해상도가 아예 다른 **별도 레이아웃**이다. 부가 투표(`#/dinner`, `#/poll/<slug>`)는 참여자 화면과 같은 375 × 812 레이아웃을 그대로 재사용한다.
