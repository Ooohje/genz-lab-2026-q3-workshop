...
사내 워크샵(60명 / 10팀)용 실시간 레크레이션 웹앱. Kahoot 대체로 만들었다.

**게임 1 — 3 Truths 1 Fake** 팀 안에서 자율 진행. 각자 진실 3 + 거짓 1을 쓰고, 팀원들이 어느 것이 거짓인지 맞힌다.
**게임 2 — 팀 대항 퀴즈** 전체가 동시에 푼다. O/X 4문항 + 4지선다 4문항.

서버는 없다. 프론트는 GitHub Pages, 백엔드는 Supabase(Postgres + Realtime) 하나로 끝낸다.

> **아래 화면은 전부 실제로 앱을 돌려서 캡처한 것이다.** 등장하는 이름·Knox ID·팀 이름·문항은 모두 데모용으로 지어낸 가상 데이터이며, 실제 참가자 정보가 아니다.
...
명단에 있으면 이름까지 자동으로 채워 바로 입장한다. 없으면 이름을 입력하고 팀 배정을 기다린다.
입장하자마자 자기 문장 4개를 쓴다. **저장하는 순간 순서가 한 번 섞여 DB에 박힌다** — 렌더링할 때마다 섞으면 팀원끼리 보이는 순서가 달라져 득표 집계가 깨지기 때문이다.
...
명단에 없는 Knox ID로 들어오면 이름을 입력하고 **팀 배정 대기** 화면에서 기다린다. 진행자가 팀을 배정하는 순간 Realtime으로 다음 화면으로 넘어간다 — 새로고침할 필요가 없고, 기다리는 동안 문장을 먼저 써둘 수 있다.

<p>
<img src="docs/screens/A3-team-wait.png" width="240" alt="팀 배정 대기 화면">
</p>
...
팀마다 **독립적으로** 진행된다. 발표자가 자기 문장 4개를 말로 설명하고, 나머지 팀원이 거짓을 고른다.
...
5초 뒤 다음 발표자로 넘어간다. 이 판정은 **서버의 조회 함수 안에서** 일어나기 때문에, 발표자 폰이 꺼져도 다른 팀원의 폴링이 대신 턴을 넘겨준다. 한 명 때문에 팀 전체가 멈추는 일이 없다.

스크린에는 10개 팀의 진행도가 한눈에 뜬다. 한 발표자에 2분 넘게 머물면 발표자 폰에 "슬슬 다음 분으로!" 배지가 뜨고, 2분 30초를 넘기면 스크린에서 그 팀이 빨갛게 반전되어 진행자가 콕 집어 독촉할 수 있다. **시스템이 강제로 끊지는 않는다** — 대화가 한창일 수도 있기 때문이다.
...
팀 점수는 단순 합계가 아니라 **팀원 개인 점수 합 ÷ 게임 2에서 1문항 이상 응답한 팀원 수**다. 접속만 하고 잠수한 인원이 분모를 부풀리지 않게 하려는 것이다. 분모는 게임 도중에도 매번 현재 편성으로 다시 계산하므로, 관리자가 팀을 옮기면 점수가 알아서 따라간다.

<img src="docs/screens/B5-screen-leaderboard.png" width="760" alt="스크린 뷰 — 중간 순위">

마지막엔 각자 자기 기록을 받는다. 게임 1에서 남의 거짓을 몇 번 맞혔는지, 우리 팀에서 누가 제일 잘 속였는지까지 나온다.
...
대시보드 하나로 행사 전체를 몬다. 단계 전환, 팀별 게임 1 진행률, 배정 대기자, 공지 배너가 한 화면에 있다. 진행이 늦은 팀은 빨간 막대로 드러나고 `강제공개`로 그 팀만 다음 발표자로 넘길 수 있다. 오른쪽 아래 "위험 구역"에 리허설 초기화와 행사 후 개인정보 삭제가 있다.

<img src="docs/screens/C2-admin-dashboard.png" width="760" alt="관리자 대시보드">

명단은 CSV로 올린다(`knox_id,name,team_no`). 팀 이동은 사람마다 붙은 드롭다운으로 하고, 바꾸는 즉시 그 사람 폰이 새 팀으로 넘어간다. `미배정`을 고르면 팀에서 빼낸다. 팀 추가는 번호만 받고(다음 번호가 미리 채워져 있다) 이름은 `N팀`으로 자동으로 붙는다. 팀을 삭제하면 그 팀원은 배정 대기로 돌아간다. 당일 오타로 들어온 사람은 이름·Knox ID를 그 자리에서 고칠 수 있다 — Knox ID를 바꿔도 이미 쓴 3T1F·투표·답안은 새 ID로 함께 옮겨간다.

<img src="docs/screens/C3-admin-roster.png" width="760" alt="관리자 명단 · 팀 편성">
...
`npm run screens`는 창을 하나 띄우고 관리자 PIN 입력을 기다린 뒤, 이 문서의 화면들을 `docs/screens/`에 다시 찍는다. 캡처를 위해 잠깐 바꾼 진행 단계와 팀 배정은 끝나면 되돌린다. 배율(2배)을 바꾸면 새로 찍은 컷만 흐릿해지므로 스크립트의 `SCALE`은 건드리지 않는다.
