...
명단은 CSV로 올린다(`knox_id,name,team_no`). 조 이동은 사람마다 붙은 드롭다운으로 하고, 바꾸는 즉시 그 사람 폰이 새 조로 넘어간다. `미배정`을 고르면 조에서 빼낸다. 조 추가는 번호만 받고(다음 번호가 미리 채워져 있다) 이름은 `N조`로 자동으로 붙는다. 조를 삭제하면 그 조원은 배정 대기로 돌아간다. 당일 오타로 들어온 사람은 이름·Knox ID를 그 자리에서 고칠 수 있다 — Knox ID를 바꿔도 이미 쓴 3T1F·투표·답안은 새 ID로 함께 옮겨간다.
